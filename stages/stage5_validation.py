...
1. Transfer Learning (TL):
   ...
   - Less robust for small datasets
   
2. Stratified K-Fold Cross Validation (S-FCV):
   - Splits data into k folds (default: k=5)
   - Maintains class distribution in each fold
   - Tests on all folds, averages results
   - More robust and reliable
   - Better for imbalanced datasets
   - Automatically adjusts k if classes too small

Outputs:
- For TL: Test accuracy and loss
- For S-FCV: 
  - Per-fold accuracies
  - Mean accuracy with standard deviation
  - Mean loss across folds

Use S-FCV for production models, TL for rapid experimentation.
'''
import numpy as np
